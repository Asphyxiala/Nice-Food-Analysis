...
# 找出综合评分最高的三家餐馆
df_top_combined = df.sort_values(by='综合评分', ascending=False).head(3)
...
for index, row in df_top_combined.iterrows():
    html_content += f'''
...
# 导出为HTML文件
html_file_path = 'top_combined_restaurants.html'
with open(html_file_path, 'w', encoding='utf-8') as file:
    file.write(html_content)

print(f"综合评分最高的三家餐馆信息已导出为HTML文件：{html_file_path}")